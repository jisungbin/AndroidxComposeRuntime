...
//        assert(buffer[nodeIndex] !== newNode)
  ...
  private fun makeNodeAtIndex(
    elementIndex: Int, newElementHash: Int, newElement: E,
    shift: Int, owner: MutabilityOwnership?,
  ): TrieNode<E> {
  ...
  private fun mutableMoveElementToNode(
    elementIndex: Int, newElementHash: Int, newElement: E,
    shift: Int, owner: MutabilityOwnership,
  ): TrieNode<E> {
    if (ownedBy === owner) {
  ...
  private fun makeNode(
    elementHash1: Int, element1: E, elementHash2: Int, element2: E,
    shift: Int, owner: MutabilityOwnership?,
  ): TrieNode<E> {
    if (shift > MAX_SHIFT) {
...
//        assert(buffer.size > 1)
  ...
  private fun mutableCollisionRetainAll(
    otherNode: TrieNode<E>, intersectionSizeRef: DeltaCounter,
    owner: MutabilityOwnership,
  ): Any? {
    if (this === otherNode) {
      intersectionSizeRef += buffer.size
  ...
  fun mutableRemoveAll(
    otherNode: TrieNode<E>, shift: Int,
    intersectionSizeRef: DeltaCounter,
    mutator: PersistentHashSetBuilder<*>,
  ): Any? {
    if (this === otherNode) {
