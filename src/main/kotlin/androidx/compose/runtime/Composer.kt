 ...
 *
 * 컴포저가 리컴포지션 스코프를 다시 컴포즈할 때 호출할 함수를 갱신하기 위해 사용되는 내부
 * Compose 컴파일러 플러그인 API입니다. 직접 사용하거나 호출해서는 안 됩니다.
 */
@ComposeCompilerApi interface ScopeUpdateScope {
   ...
   *
   * 생성된 코드에서 리컴포지션 스코프를 갱신하기 위해 호출되며, 스코프를 다시 컴포즈할 때
   * 사용할 함수를 설정합니다. 이는 Compose 컴파일러 플러그인이 생성한 코드에 의해 호출되며
   * 직접 호출해서는 안 됩니다.
   */
 ...
 *
 * 그룹을 재사용하거나 비활성화하는 동안 제거되지 않는 Remember 옵저버입니다.
 * 이 옵저버는 그룹 비활성화 사이에서 CompositionLocal을 보존하는 데 사용됩니다.
 */
