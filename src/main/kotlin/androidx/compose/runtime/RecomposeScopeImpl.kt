...
internal class RecomposeScopeImpl(internal var owner: RecomposeScopeOwner?) :
   ...
   *
   * 스코프의 컴포지션을 재시작합니다. [block]이 갱신되지 않았다면 오류가 발생합니다. 컴파일러가
   * 생성한 코드는 리컴포즈 스코프가 사용될 때 [block]이 설정되도록 보장하지만, 컴파일러와
   * 런타임 버전이 맞지 않거나, [Composer.startRestartGroup] 및 [Composer.endRestartGroup]을
   * 잘못 직접 호출한 경우 이런 상황이 발생할 수 있습니다.
   */
  // MEMO 리컴포지션 실행 함수
  ...
  override fun invalidate() {
    owner?.invalidate(scope = this, instance = null)
  }
