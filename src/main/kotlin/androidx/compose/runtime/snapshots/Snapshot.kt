...
import androidx.compose.runtime.snapshots.Snapshot.Companion.PreexistingSnapshotId
import androidx.compose.runtime.snapshots.Snapshot.Companion.notifyObjectsInitialized
import androidx.compose.runtime.snapshots.Snapshot.Companion.registerGlobalWriteObserver
...
sealed class Snapshot(
  ...
  companion object {
     ...
     * state.
     */
    @Suppress("ConstPropertyName") const val PreexistingSnapshotId: Int = 1
    ...
    @InternalComposeApi fun openSnapshotCount(): Int = openSnapshots.toList().size
...
abstract class StateRecord(
  ...
  @Deprecated("Use snapshotId: Long constructor instead") constructor(id: Int) : this(id.toSnapshotId())
 ...
 */
@JvmDefaultWithCompatibility interface StateObject {
...
private var nextSnapshotId = PreexistingSnapshotId.toSnapshotId() + 1
...
internal fun <T : StateRecord> T.writableRecord(state: StateObject, snapshot: Snapshot): T {
  ...
  if (readData.snapshotId != PreexistingSnapshotId.toSnapshotId()) {
    snapshot.recordModified(state)
  }
...
internal fun <T : StateRecord> T.overwritableRecord(
  ...
  if (candidate.snapshotId != PreexistingSnapshotId.toSnapshotId()) {
    snapshot.recordModified(state)
  }
